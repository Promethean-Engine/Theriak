...
const Content: React.FC = () => {
  ...
  return (
    <div className="contentContainer">
      ...
      />
      <div className="icons">
        {!isTrustPeopleVisibile && (
          <FontAwesomeIcon
            icon={faUserCircle}
            onClick={() => setIsTrustPeopleVisible(true)}
          />
        )}
        {!isAddEpiVisible && (
          <FontAwesomeIcon
            ...
            onClick={() => setisAddEpiVisible(true)}
          />
        )}
        {!isAddTrustVisible && (
          <FontAwesomeIcon
            ...
            style={{ marginLeft: "10px" }}
            onClick={() => setisAddTrustVisible(true)}
          />
        )}
      </div>
      {isTrustPeopleVisibile && (
        <TrustPeople
          people={trustPeople}
          isLoadingData={isTrustPeopleLoading}
          closeTrustPeople={() => setIsTrustPeopleVisible(false)}
        />
      )}
      {isAddEpiVisible && (
        <AddEpiModal closeModal={() => setisAddEpiVisible(false)} />
      )}
      {isAddTrustVisible && (
        <AddTrustModal closeModal={() => setisAddTrustVisible(false)} />
      )}
    </div>
  );
};
